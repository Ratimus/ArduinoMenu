...
#ifdef MENU_ASYNC
template<typename T> inline const char* typeStr()
{
  return "o";
}

template<> inline const char* typeStr<uint8_t>()
{
  return "u8";
}

template<> inline const char* typeStr<uint16_t>()
{
  return "u16";
}

template<> inline const char* typeStr<uint32_t>()
{
  return "u32";
}

template<> inline const char* typeStr<int8_t>()
{
  return "i8";
}

template<> inline const char* typeStr<int16_t>()
{
  return "i16";
}

template<> inline const char* typeStr<int32_t>()
{
  return "i32";
}
...
template<> inline const char* typeStr<float>()
{
  return "f";
}

template<> inline const char* typeStr<double>()
{
  return "d";
}
#endif
...
public:
  ...
  inline prompt(constMEM promptShadow& shadow):
    shadow(&shadow)
  {}

  inline prompt(constText* t,
                action a = doNothing,
                eventMask e = noEvent,
                styles s = noStyle,
                systemStyles ss = _noStyle):
  ...
  inline void disable()
  {
    enabled = disabledStatus;
  }

  inline constText* getText() const
  {
    return shadow->getText();
  }

  inline systemStyles sysStyles() const
  {
    return shadow->_sysStyles();
  }

  inline styles style() const
  {
    return shadow->_style();
  }

  inline eventMask events() const
  {
    return shadow->_events();
  }


  virtual idx_t sz() const
  {
      return 0;
  }

  inline bool is(systemStyles chk)  const
  {
      return (sysStyles()&chk) == chk;
  }

  inline bool has(systemStyles chk) const
  {
      return sysStyles()&chk;
  }

  inline bool is(styles chk)        const
  {
      return (style()&chk) == chk;
  }

  inline bool has(styles chk)       const
  {
      return style()&chk;
  }

  inline bool is(eventMask chk)     const
  {
      return (events()&chk) == chk;
  }

  inline bool has(eventMask chk)    const
  {
      return events() & chk;
  }

  inline bool canWrap()    const
  {
      return style() & wrapStyle;
  }

  inline bool canNav()     const
  {
      return sysStyles() & _canNav;
  }//can receive navigation focus and process keys

  inline bool isMenu()     const
  {
      return sysStyles() & _menuData;
  }//has menu data list and can be a navNode target

  inline bool isVariant()  const
  {
      return sysStyles() & _isVariant;
  }//a menu as an enumerated field, connected to a variable value

  inline bool parentDraw() const
  {
      return sysStyles() & _parentDraw;
  }//a menu as an enumerated field, connected to a variable value

  inline bool asPad()      const
  {
      return sysStyles() & _asPad;
  }//a menu as an enumerated field, connected to a variable value

  inline bool hasTitle(navNode& nav) const;

  inline result operator()(eventMask event, navNode& nav, prompt &item) const
  {
      return (*shadow)(event, nav, item);
  }
  ...
  virtual result sysHandler(eventMask event, navNode& nav, prompt &item)
  {
      return proceed;
  }
...
{
public:
  inline Exit(constText* t):
    prompt(t, (callback)doExit, enterEvent)
  {}
};
...
public:
  ...
  inline navTarget(constMEM promptShadow& shadow):
    prompt(shadow)
  {}

  inline navTarget(constText* t,
                   action a         = doNothing,
                   eventMask e      = noEvent,
                   styles s         = noStyle,
                   systemStyles ss  = _noStyle):
...
public:
  ...
  inline textField(constMEM textFieldShadow& shadow):
    navTarget(shadow)
  {}

  inline textField(constText*label,
                   char* b,
                   idx_t sz,
                   char* const* v,
                   action a         = doNothing,
  ...
  {
      return ((textFieldShadow*)shadow)->_buffer();
  }

  inline idx_t sz() const
  {
      return ((textFieldShadow*)shadow)->_sz();
  }
...
public:
  ...
  inline fieldBase(constMEM promptShadow& shadow):
    navTarget(shadow)
  {}
  ...
  inline constText* units()
  {
      return ((fieldBaseShadow*)shadow)->_units();
  }
...
public:
  ...
  T reflex;
  menuField(constMEM menuFieldShadow<T> & shadow):
    fieldBase(shadow)
  {}
  ...
  menuField(T &value,
            ...
            eventMask e = noEvent,
            styles s    = noStyle):
    menuField(*new menuFieldShadow<T>(value,
                                      text,
                                      units,
                                      low,
                                      high,
  ...
  {
    return ((menuFieldShadow<T>*)shadow)->target();
  }

  inline T getTypeValue(constMEM T* from) const
  {
    return ((menuFieldShadow<T>*)shadow)->getTypeValue(from);
  }

  inline T low() const
  {
    return  ((menuFieldShadow<T>*)shadow)->_low();
  }

  inline T high() const
  {
    return ((menuFieldShadow<T>*)shadow)->_high();
  }

  inline T step() const
  {
    return ((menuFieldShadow<T>*)shadow)->_step();
  }

  inline T tune() const
  {
    return ((menuFieldShadow<T>*)shadow)->_tune();
  }
...
public:
  ...
  inline menuValue(constMEM menuValueShadow<T>& shadow):
    prompt(shadow)
  {}

  inline menuValue(constText* text,
                   T value,
                   action a = doNothing,
                   eventMask e = noEvent)
  ...
  inline T target() const
  {
      return ((menuValueShadow<T>*)shadow)->target();
  }
...
{
public:
  inline menuNode(constMEM menuNodeShadow& s):
    navTarget(s)
  {}

  inline menuNode(const char* text,
                  idx_t sz,
                  prompt* constMEM data[],
                  action a = noAction,
                  eventMask e = noEvent,
                  styles style = wrapStyle,
  ...
  virtual prompt& operator[](idx_t i) const
  {
    return ((menuNodeShadow*)shadow)->operator[](i);
  }
  bool changed(const navNode &nav, const menuOut& out, bool sub = true, bool test = false) override;
  void clearChanged(const navNode &nav, const menuOut& out, bool sub) override;
  inline idx_t sz() const
  {
    return ((menuNodeShadow*)shadow)->_sz();
  }
  inline prompt* constMEM* data() const
  {
    return ((menuNodeShadow*)shadow)->_data();
  }
...
public:
  ...
  inline menu(constMEM menuNodeShadow& shadow):
    menuNode(shadow)
  {}
...
{
public:
  inline menuVariantBase(constMEM menuNodeShadow& s):
    menuNode(s)
  {}
...
public:
  ...
  idx_t reflex;
  inline menuVariant(constMEM menuNodeShadow& s):
    menuVariantBase(s)
  {}
  ...
  inline T& target() const
  {
    return ((menuVariantShadow<T>*)shadow)->target();
  }
...
public:
  ...
  inline toggle(constMEM menuNodeShadow& s):
    menuVariant<T>(s)
  {}

  inline toggle(constText* text,
                T &target,
                idx_t sz,
                prompt* constMEM* data,
                action a = doNothing,
