...
template<typename T>
  const char* menuValue<T>::typeName() const
{
  return "menuValue";
}
#endif
...
template<typename T>
  bool menuField<T>::canTune()
{
  return !!tune();
}

template<typename T>
  void menuField<T>::constrainField()
{
...
template<typename T>
  idx_t menuField<T>::printReflex(menuOut& o) const
{
...
#ifdef MENU_ASYNC
template<typename T>
  void menuField<T>::printValue(menuOut& o) const
{
  o.print(reflex);
}

template<typename T>
  void menuField<T>::printHigh(menuOut& o) const
{
  o.print(high());
}

template<typename T>
  void menuField<T>::printLow(menuOut& o) const
{
  o.print(low());
}

template<typename T>
  void menuField<T>::printStep(menuOut& o) const
{
  o.print(step());
}

template<typename T>
  void menuField<T>::printTune(menuOut& o) const
{
  o.print(tune());
}

template<typename T>
  const char* menuField<T>::typeName() const
{
  return typeStr<T>();
}
#endif
...
{
  if (options->invertFieldKeys)
  {
    dir *= -1;
  }

  T thisstep = tuning ? tune() : step();
  dirty = true;
...
{
  return o.print(Menu::menuField<T>::reflex,dps);
}
  ...
  Used choose<T>::printTo(navRoot &root,bool sel,menuOut& out, idx_t idx,idx_t len,idx_t panelNr)
{
  ...
  if (ed)
  {
    return prompt::printTo(root,sel,out,idx,len,panelNr);
  }

  return menuVariantBase::printTo(root,sel,out,idx,len,panelNr);
}
...
#ifdef MENU_ASYNC
template<typename T>
  const char* toggle<T>::typeName() const
{
  return "toggle";
}
...
template<typename T>
  result toggle<T>::sysHandler(eventMask event, navNode& nav, prompt &item)
{
  ...
  switch(event)
  {
    case activateEvent:
    {
      ...
      prompt::dirty = true;
      (menuNode::operator[](at))(event, nav, item);
      // Ratimus: Fall through on purpose?
    }
    default:
    {
      return proceed;
...
#ifdef MENU_ASYNC
template<typename T>
  idx_t menuVariant<T>::selected() const
{
  return reflex;
}

template<typename T>
  const char* menuVariant<T>::typeName() const
{
  return "menuVariant";
}
#endif
...
template<typename T>
  result choose<T>::sysHandler(eventMask event, navNode& nav, prompt &item)
{
  switch(event)
  {
    case updateEvent:
...
#ifdef MENU_ASYNC
template<typename T>
  const char* choose<T>::typeName() const
{
  return "choose";
}
#endif
