...
////////////////////////////////////////////////////////////////////////////////
  ...
  template <typename T>
  menuOut& menuOut::operator << (T o)
  {
    (*(Stream*)this)<< (o);return *this;
  }
  #endif
#endif
...
void menuOut::drawCursor(idx_t ln, bool selected, status stat, bool edit, idx_t panelNr)
{
  ...
  {
    write(options->disabledCursor);
    return;
  }

  write(options->selectedCursor);
}

idx_t menuOut::startCursor(navRoot& root, idx_t x, idx_t y, bool charEdit, idx_t panelNr)
{
  write(charEdit ? ">" : "[");
  return 1;
}

idx_t menuOut::endCursor(navRoot& root, idx_t x, idx_t y, bool charEdit, idx_t panelNr)
{
  write(charEdit ? " < " : "]");
  return 1;
}
...
{
  setColor(fgColor, true, enabledStatus, true);
  return 0;
}
...
Used outputsList::printMenu(navNode& nav) const
{
  ...
  for (int n = 0; n < cnt; n++)
  {
    ...
    if ((o.style & (menuOut::rasterDraw))
     || (o.style & (menuOut::redraw))
     || nav.changed(o))
    {
      o.printMenu(nav);
    }
...
void outputsList::refresh()
{
  for(int n = 0; n < cnt; n++)
  {
    ((menuOut*)memPtr(outs[n]))->drawn = NULL;
  }
}
...
void outputsList::clearChanged(navNode& nav) const
{
  for(int n = 0;n < cnt;n++)
  {
    ((menuOut*)memPtr(outs[n]))->clearChanged(nav);
  }
}
...
void outputsList::doNav(navCmd cmd, class navNode &nav)
{
  for(int n = 0; n < cnt; n++)
  {
    ((menuOut*)memPtr(outs[n]))->doNav(cmd, nav);
  }
}
...
Used menuOut::printMenu(navNode &nav, idx_t panelNr)
{
  ...
  else
  {
    ...
    if(topi && nav.root->active().parentDraw())
    {
      topi--;
    }
    ...
    idx_t st = ((nav.target->style() & showTitle)
    || (nav.root->showTitle && (!(nav.target->style() & noTitle))))
    ...
    if (!(all || (style & minimalRedraw)))
    {
      //non minimal draw will redraw all if any change
    ...
    for(idx_t i = 0; asPad || (i < maxY(panelNr) - st); i++)
    {
      ...
      if (all || p.changed(nav, *this, false))
      {
        ...
        if (!asPad)
        {
          //pad MUST be able of clearing part of a line!!!!!
        ...
        if (nav.root->useAccel && (!asPad) && (drawNumIndex&style))
        {
          //<-- NO INDEX FOR PADS
          char a= top + i + '1';
          print('[');
        ...
        if (len>0)
        {
          len= p.printTo(*nav.root, selected, *this, i, len, panelNr);
        }
