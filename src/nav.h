...
{
  idx_t x,y,w,h;
  inline idx_t maxX() const
  {
    return x+w;
  }

  inline idx_t maxY() const
  {
    return y+h;
  }
};
...
//multiple panels
// Ratimus: I think this is a list of all the available screens?
class panelsList
{
public:
  const panel* panels;
  ...
  constMEM idx_t sz;
  idx_t cur = 0;
  inline panelsList(const panel p[],
                    navNode* nodes[],
                    idx_t sz):
    panels(p),
    nodes(nodes),
...
protected:
  ...
  inline menuNodeShadow& shadow() const
  {
    return *(menuNodeShadow*)target->shadow;
  }

  inline void useMenu(menuNode &menu)
  {
    target = &menu;
    reset();
  }
...
public:
    ...
    inline prompt& selected() const
    {
      return *(prompt*)memPtr(data()[sel]);
    }

    inline bool wrap() const
    {
      return target->style()&wrapStyle;
    }

    result event(eventMask e,idx_t i);//send event to item index i

    inline result event(eventMask e)
    {
      return event(e,sel);
    }//send event to current item

    result sysEvent(eventMask e,idx_t i);//send system event to item index i

    inline result sysEvent(eventMask e)
    {
      return sysEvent(e,sel);
    }//send event to current item

    navCmd navKeys(char ch);
    navCmd doNavigation(navCmd cmd);//aux function
    // inline int getPanelIdx(menuOut& out) const
    //{
    // return out.getPanelIdx(*this);
    //}
...
class navRoot
{
  public:
    ...
    inline navNode& node() const
    {
      return path[level];
    }

    inline menuNode& active() const
    {
      return *node().target;
    }
    ...
    inline prompt& selected() const
    {
      return active()[node().sel];
    }

    bool changed(const menuOut& out);

    inline bool changed(idx_t n)
    {
  ...
  inline Stream& operator<<(Stream&o,const navRoot& p)
  {
    return o<<p.node();
  }
#endif
