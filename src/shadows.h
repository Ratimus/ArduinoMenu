...
public:
  ...
  {
    return (constText*)memPtr(text);
  }

  inline systemStyles _sysStyles() const
  {
    return (systemStyles)memEnum(&sysStyles);
  }

  inline eventMask _events() const
  {
    return (eventMask)memEnum(&events);
  }

  inline styles _style() const
  {
    return (styles)memEnum(&style);
  }
};
...
public:
  ...
  idx_t _sz() const
  {
    return (idx_t)memIdx(sz);
  }

  char* _buffer() const
  {
    return (char*)memPtr(buffer);
  }

  char* const* _validators() const
  {
    return (char* const*)memPtr(validators);
  }
...
public:
  ...
  menuNodeShadow(const char* text,
                 idx_t sz,
                 prompt* constMEM* data,
                 action a,
                 eventMask e,
  ...
  prompt* constMEM* _data() const
  {
    return (prompt* constMEM*)memPtr(data);
  }
...
public:
  constText* units;
  fieldBaseShadow(constText   *text,
                  constText   *units,
                  action       a  = doNothing,
                  eventMask    e  = noEvent,
                  styles       s  = noStyle,
                  systemStyles ss = ((Menu::systemStyles)(Menu::_canNav|Menu::_parentDraw))):
    promptShadow(text, a, e, s, ss),
    units(units)
  {}

  inline constText* _units()
  {
    return (constText*)memPtr(units);
  }
};
...
public:
  ...
  menuFieldShadow(T         &value,
                  ...
                  styles       s  = noStyle,
                  systemStyles ss = ((Menu::systemStyles)(Menu::_canNav|Menu::_parentDraw))):
    fieldBaseShadow(text, units, a, e, s, ss),
    value(&value),
    low(low),
    high(high),
    step(step),
  ...
  inline T _low() const
  {
    return getTypeValue(&low);
  }

  inline T _high() const
  {
    return getTypeValue(&high);
  }

  inline T _step() const
  {
    return getTypeValue(&step);
  }

  inline T _tune() const
  {
    return  getTypeValue(&tune);
  }
};
...
public:
  T value;
public:
  inline menuValueShadow(constText* text,
                         T value,
                         action    a = doNothing,
                         eventMask e = noEvent):
    promptShadow(text, a, e),
    value(value)
  {}
...
public:
  T *value;
  menuVariantShadow(constText        *text,
                    T                &target,
                    idx_t             sz,
                    prompt *constMEM *data,
                    action            a,
                    ...
                    styles            style,
                    systemStyles ss = (systemStyles)(_menuData|_canNav)):
    menuNodeShadow(text, sz, data, a, e, style, ss),
    value(&target)
  {}
