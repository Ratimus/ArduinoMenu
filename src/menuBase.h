...
struct navCmd
{
  ...
  inline bool operator==(navCmds n) const
  {
    return cmd==n;
  }

  inline operator navCmds() const
  {
    return cmd;
  }
};
...
enum status
{
  disabledStatus = 0, enabledStatus = 1
};

//callback function type
typedef result (*callback)(eventMask event, navNode& nav, prompt &item);
typedef void (*vCall)(eventMask event, navNode& nav, prompt &item);//now also accept void return
...
public:
  ...
  {
    return ((callback)memPtr(hFn))(event, nav, item);
  }
};
...
struct config
{
  ...
  inline char getCmdChar(navCmds cmd) const
  {
    return navCodes[cmd].ch;
  }//return character assigned to this command
};
...
typedef class Area
{
  ...
  public:
    ...
    inline Area  operator+ (const Area& o) const
    {
      return op(&Area::operator+=, o);
    }
