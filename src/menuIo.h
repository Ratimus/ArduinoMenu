...
public:
  ...
  inline StringStream(const char*s):
    src(s)
  {;}
...
public:
  ...
  inline menuOut(idx_t *topsList,
                 panelsList &p,
                 styles os = minimalRedraw):
  ...
  idx_t printText(const __FlashStringHelper* at, idx_t len)
  {
      return printRaw(at, len);
  }
  ...
  void previewMenu(navRoot& root,
                   menuNode& menu,
                   idx_t panelNr);//draw a preview on a panel

  Used printMenu(navNode &nav);//print menus and previews on panels

  void doNav(navCmd cmd, navNode &nav);

  virtual void clearLine(idx_t ln,
                         idx_t panelNr = 0,
                         colorDefs color = bgColor,
                         bool selected = false,
                         status stat = enabledStatus,
  ...
  virtual void setCursor(idx_t x,
                         idx_t y,
                         idx_t panelNr = 0) = 0;
  ...
  virtual void rect(idx_t panelNr,
                    idx_t x,
                    idx_t y,
                    idx_t w = 1,
                    idx_t h = 1,
  ...
  virtual void box(idx_t panelNr,
                   idx_t x,
                   idx_t y,
                   idx_t w = 1,
                   idx_t h = 1,
...
public:
    ...
    inline gfxOut(idx_t rx,
                  idx_t ry,
                  idx_t* t,
                  panelsList &p,
                  menuOut::styles st = menuOut::minimalRedraw,
                  idx_t fontMarginY = 1):
        menuOut(t, p, st),
        resX(rx),
        resY(ry),
        fontMarginY(fontMarginY)
    {;}
...
class outputsList
{
  public:
    ...
    outputsList(menuOut* constMEM o[], int n):
        cnt(n),
        outs(o)
    {;}
...
inline idx_t menuOut::maxX(idx_t i) const
{
  return panels[i].w;
}

inline idx_t menuOut::maxY(idx_t i) const
{
  return panels[i].h;
}
