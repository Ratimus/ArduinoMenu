...
void panelsList::reset(idx_t from)
{
  for(int n = from; n < sz; n++)
  {
    nodes[n] = NULL;
  }
}
...
navCmd navNode::navKeys(char ch)
{
  ...
  {
    if (options->navCodes[i].ch == ch)
    {
      return options->navCodes[i].cmd;
    }
  }
  return noCmd;
}
...
navCmd navNode::doNavigation(navCmd cmd)
{
  ...
  if((osel != nsel) || changed)
  {
    //selection changed, must have been and idx/sel or an up/down movement
    ...
    if (selBlurEvent & target->events())
    {
      target->operator()(selBlurEvent, *this, *target);
    }

    event(blurEvent, osel);
    sel = nsel;

    if (cmd.cmd == selCmd || cmd.cmd == idxCmd)
    {
      //do accelerator and enter the option
      assert(root);
      rCmd = root->enter();
    }//other commands up/down just receive focus events

    event(focusEvent, nsel);
    if (selFocusEvent&target->events())
    {
      target->operator()(selFocusEvent, *this, *target);
    }
...
result navNode::event(eventMask e, idx_t i)
{
  ...
  if (me)
  {
    return p.eventHandler(e, *this, i);
  }//p(e, p);
  return proceed;
}
...
void navRoot::doInput(menuIn& in)
{
  ...
  {
    char c = in.read();
    if (options->getCmdChar(enterCmd) == c || options->getCmdChar(escCmd) == c)
    {
      idleOff();
    }
    else
  ...
  {
    trace(MENU_DEBUG_OUT << "navRoot::doInput (available = TRUE)" << endl);
    //if not doing something else and there is input
...
void navRoot::doOutput()
{
  ...
  else
  {
    ...
    if (idleTask != sleepTask)
    {
      out.idle(idleTask, idling);
    }
#endif
  }
}
...
void navRoot::doNav(navCmd cmd)
{
  if (sleepTask)
  {
    if (cmd.cmd == enterCmd || cmd.cmd == escCmd)
    {
      idleOff();
    }
    return;
...
navCmd navRoot::enter()
{
  ...
  if (!selected().enabled)
  {
    ...
    _trace(MENU_DEBUG_OUT << "!selected().enabled" << endl);
    return noCmd;
  }

  if (selected().sysHandler(activateEvent, node(), selected()) != proceed)
  {
    Serial.println("no proceed");
    _trace(MENU_DEBUG_OUT << "selected().sysHandler(activateEvent, node(), selected()) != proceed" << endl);
    return noCmd;
  }
...
// {
  ...
  else if (go == quit && !selected().isMenu())
  {
    exit();
  }
...
navCmd navRoot::exit()
{
  _trace(MENU_DEBUG_OUT << "navRoot::exit" << endl);
  navFocus->dirty = true;
  if (navFocus->isMenu())
  {
    if (level)
...
{
  trace("ENTER navNode::changed");
  if (out.drawn == NULL
  || target->changed(*this, out))
  {
  ...
  if (root->canExit
    && root->timeOut
    && ((millis() - root->lastChanged) / 1000) > root->timeOut)
  {
...
bool navRoot::changed(const menuOut& out)
{
  ...
  navNode& thisNode = node();
  bool ret = thisNode.changed(out);

  if (ret) trace(MENU_DEBUG_OUT.println("navRoot::changed()"));
  return ret;
}
...
bool navRoot::async(const char* at)
{
  ...
  {
    return active().async(at, *this, level+1);
  }//TODO: check max_depth here!
}
