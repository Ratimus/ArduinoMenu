...
bool prompt::hasTitle(navNode& nav) const
{
  return (nav.target->has(showTitle)||(nav.root->showTitle&&!nav.target->has(noTitle)));
}
...
Used textField::printTo(navRoot &root,bool sel,menuOut& out, idx_t idx,idx_t len,idx_t panelNr)
{
  ...
  {//while not string terminated or buffer length reached
...
#ifdef MENU_ASYNC
  const char* menuNode::typeName() const
  {
    return "mn";
  }
#endif
...
void fieldBase::doNav(navNode& nav,navCmd cmd)
{
  switch(cmd.cmd)
  {
    ...
    case escCmd:
      tuning=true;//prepare for exit
    case enterCmd:
      if (tuning||nav.root->nav2D||!canTune())
      {//then exit edition
        tuning=false;
        dirty=true;
        constrainField();
...
Used fieldBase::printTo(navRoot &root,bool sel,menuOut& out, idx_t idx,idx_t len,idx_t panelNr)
{
  ...
  if (l<len)
  {
    ...
    out.print((root.navFocus==this&&sel)?(tuning?'>':':'):' ');
    l++;
    if (l<len)
    {
      #ifdef MENU_FMT_WRAPS
...
bool menuNode::_changes(const navNode &nav, const menuOut& out, bool sub, bool test)
{
  ...
  if (parentDraw())
  {
    ...
    nav.root->path[level-1].target->clearChanged(nav.root->path[level - 1], out, sub);
  }

  idx_t my = out.maxY();
  if (has(showTitle) || (nav.root->showTitle && !has(noTitle)))
  {
    my -= 1;
  }
  ...
  idx_t tmpIdx = level;
  if (target->has(_asPad) || target->has(_parentDraw))
  {
    tmpIdx -= 1;
  }
  idx_t t = out.tops[level - tmpIdx];
  trace(MENU_DEBUG_OUT << "t:" << t << endl;);

  if (sub)
  {
    for(int i = 0; i < my && t < sz(); i++, t++)
    {
      // if (t >= sz())
      // {
      //   break;
...
#ifdef MENU_ASYNC
const char* menu::typeName() const
{
  return "menu";
}
#endif
...
#ifdef MENU_ASYNC
const char* menuVariantBase::typeName() const
{
  return "menuVariantBase";
}
#endif
